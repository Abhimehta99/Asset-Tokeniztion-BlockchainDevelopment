...
contract("TokenSale", async function(accounts){
    ...
    it("should be possible to buy one token by simply sending ether to the smart contract", async()=>{
        ...
        expect(balanceBeforeAccount + 1).to.be.bignumber.equal(balanceBeforeAccount);

        let kycInstance=await KycContract.deployed();
        await kycInstance.setKycCompleted(recipient);
        expect(tokenSaleInstance.sendTransaction({from: recipient, value: web3.utils.toWei("1", "wei")})).to.be.fulfilled;
        return expect(balanceBeforeAccount + 1).to.be.bignumber.equal(balanceBeforeAccount);
    });
});
